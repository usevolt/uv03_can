...
/// Blocks while the parameters are read over SDO. Returns true on success.
bool saveparam_save_device(device_st *device, const char *file);


/// @brief: Starts saveparam_save_device() on its own task so the caller (the UI)
/// is not blocked while parameters are read over SDO. Poll
/// saveparam_is_finished(&dev.saveparam) for completion.
