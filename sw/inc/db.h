...
/// @brief: Deinitializes the database: frees all allocated memory and resets it
/// to an empty, not-loaded state (db_is_loaded() returns false afterwards).
/// Safe to call multiple times. Also used to unload a database that was loaded
/// only temporarily (e.g. to read identification fields), so the rest of the
/// program still sees "no database loaded".
void db_deinit(void);
