...
/// @brief: Asks the device to forward only what a parameter transfer needs —
/// the SDO conversation, plus the control frames that say the node is there at
/// all — instead of everything the filters above would let through.
///
/// This is the one filter that cannot be expressed as an id range, because it
/// is about what a frame is for rather than what it is numbered: the device
/// classifies its own bus (see remote_can_class_of) and leaves the process data
/// at home. That is nearly all of a machine bus and none of what a load or a
/// save of the parameters reads, so on a link as thin as this one it is the
/// difference between a transfer that finishes and one that crawls.
///
/// Only the device's side is narrowed. What is written to the interface at this
/// end is generated by this tool's own commands, it is sparse already, and
/// silently dropping a frame somebody deliberately sent would be a surprise.
///
/// Changing this while a bridge is up takes effect on the device at once.
